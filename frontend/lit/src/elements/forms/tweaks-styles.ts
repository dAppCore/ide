// Shared styles for the tweaks-panel family (Panel, Section, Row,
// Slider, Toggle, Radio, Select, Text, Number, Color, Button).
// Ported verbatim from tweaks-panel.jsx __TWEAKS_STYLE so the visual
// fidelity matches the design source 1:1.
//
// All `.twk-*` class names are deliberately uniquely-prefixed; safe to
// inject globally via the panel, OR include in each control's static
// styles when they live in shadow DOM.

import { css } from 'lit';

export const tweaksStyles = css`
  .twk-panel {
    position: fixed;
    right: 16px;
    bottom: 16px;
