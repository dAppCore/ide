...
export class IdeComponent implements OnInit, OnDestroy {
  ...
  mantisLoading = signal(false);
  mantisSelected = signal<any | null>(null);
  mantisInspectLoading = signal(false);
  mantisStatusFilter = signal('');
  mantisVisible = computed(() => {
  ...
  async openMantisIssue(id: number) {
    ...
    try {
      const res = await this.bridgeCall('mantis_view', { id });
      if (res.ok) this.mantisSelected.set(res.value);
    } finally {
