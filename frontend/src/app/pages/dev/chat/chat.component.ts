...
import * as ChatBridge from '../../../../../bindings/dappco.re/go/ide/pkg/server/chatbridge';
import {
  ChatModelOutput as ChatModel,
  ChatMessageOutput as ChatMessage,
  ChatConversationOutput as ChatConversation,
} from '../../../../../bindings/dappco.re/go/ide/pkg/server/models';

// Migrated 2026-05-09 from callBridge('chat_*') to typed ChatBridge
// wails bindings — direct goroutine calls, autogenerated TS types
// from go/pkg/server/chatpanelbridge.go (extends the existing
// chatBridge struct used by the right-rail Vi shell).
 ...
 * as toasts.
 *
 * Migrated to typed ChatBridge wails bindings (was callBridge('chat_*')).
 * Streaming still goes through the same request/response loop — SSE
 * for token streaming is a future lane once the chat service exposes
...
export class ChatComponent implements OnInit {
  ...
  async loadConversation(id: string): Promise<void> {
    ...
    try {
      const v = await ChatBridge.ConversationsLoad(id);
